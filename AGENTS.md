...
- The Jellyfin catalog lives in `src/catalog.rs` (artists/albums/tracks/playlists, stable UUIDv5 ids derived from Spotify URIs). `src/spotify.rs` populates it by evaluating one collector script in the renderer via the bridge (`RootlistAPI.getContents` + `PlaylistAPI.getPlaylist` per playlist); a background task refreshes it at boot and every 15 min.
