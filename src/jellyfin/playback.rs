...
pub fn playback_info_get(item_id: Uuid, state: &State<AppState>) -> Json<Value> {
    playback_info(item_id, state)
}

#[post("/Items/<item_id>/PlaybackInfo", data = "<_profile>")]
pub fn playback_info_post(item_id: Uuid, _profile: Json<Value>, state: &State<AppState>) -> Json<Value> {
    playback_info(item_id, state)
}

fn playback_info(item_id: Uuid, state: &State<AppState>) -> Json<Value> {
